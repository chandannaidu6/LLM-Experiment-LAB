...
def _build_row(result:RunResult):Dict[str,Any]:
    ...
    cost_tracked = result.cost.get("total_tokens",0) > 0 or (result.retriever_name == "bm25")
...
def print_comparison(result:List[RunResult],sort_by:str = "recall_at_k",console:Optional[Console] = None)->None:
    console = console or Console()
    rows = [_build_row(r) for r in results]
    def sort_key(r):
        if r["status"] == "ok":
            val = r.get(sort_by)
            return (0,-(val if isinstance(int,float) else 0))
